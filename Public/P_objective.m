        ...
        case 'value'
            ...
            switch Problem
                ...
                case 'DTLZ5'
                    ...
                    FunctionValue = repmat(1+g,1,M).*fliplr(cumprod([ones(size(g,1),1),cos(PopDec(:,1:M-1)*pi/2)],2)).*[ones(size(g,1),1),sin(PopDec(:,M-1:-1:1)*pi/2)];     
                case 'DTLZ6'
                    PopDec = Population;
...
function [Output,Boundary,Coding] = P_SDTLZ(Operation,Problem,M,Input)
    ...
    switch Operation
        ...
        case 'value'
            ...
            FunctionValue(:,1) = FunctionValue(:,1)*1;
            FunctionValue(:,2) = FunctionValue(:,2)*1;
            FunctionValue(:,3) = FunctionValue(:,3)*20;
            Output = FunctionValue;
            %Output = FunctionValue.*repmat((F.^(0:M - 1)), [size(FunctionValue,1) 1]);%0:M - 1
        %Sample True PFs
        case 'true'
            switch Problem
                case 'SDTLZ1'
            ...
            Population(:,3) = Population(:,3)*20;
            Output = Population;
            %Output = Population.*repmat((F.^(0:M - 1)), [size(Population,1) 1]);%2*ones(1,M)
    end
end
